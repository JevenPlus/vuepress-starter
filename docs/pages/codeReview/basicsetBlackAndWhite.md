...
export default {
  ...
  methods: {
    ...
    getData_k({customerCodes = [], unselectedCustomerCodeList = [], isEnabled = '2'}) {
      /*zkl*/
      ...
      this.sunselectedCustomerCodeListInit = JSON.parse(JSON.stringify(unselectedCustomerCodeList));
      this.scustomerCodeListAll = [...customerCodes, ...unselectedCustomerCodeList]
    ...
    queryDataBelongInfo() {
      /*zkl*/
      this.sunselectedCustomerCodeListSearch = this.sunselectedCustomerCodeList.filter(
        (item) =>
          item.customerCode &&
    ...
    resetWhiteList() {
      ...
      this.scustomerCodeList = JSON.parse(JSON.stringify(this.scustomerCodeListInit))
      this.scustomerCodeListSearch = this.scustomerCodeList
      // 重置isEnabled
...
2. 高频的操作应抽出来做一个公共方法, 并放到common里面, 方便小组成员查看与复用
